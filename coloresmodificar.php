<?php 
    ...
    $id = trim($_GET['id']);
    $qry = "select * from colores where id=$id";
    //echo $qry."<br>";
...
<html lang="en">
    ...
    <form action="metodos.php?metodo=coloresmodificar" method="POST" onsubmit="return valida_campos()">
        <table class="styled-table-1">
            <thead>
                <tr>
                    <th colspan="2">MODIFICAR DATOS DEL COLOR</th>
                </tr>
            </thead>
            <tbody>
                <tr>
                    <td>DESCRIPCION:</td>
                    <td><input type="text" id="descripcion" name="descripcion" value="<?php echo $descripcion?>" size="30" maxlength="30" oninput="convertirDescripcionAMayusculas()" style="text-transform: uppercase;">
        ...
        <table class="tablabotones">
            <tr>
                ...
                <td><input type="button" name="cancelar" id="cancelar" class = "boton" value="CANCELAR" title="CANCELAR"  onclick="javascript:{lanzar('coloresvista.php')}"></td>
            </tr>
        </table>
    </form>
</html>
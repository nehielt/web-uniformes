<?php 
    ...
    $id = trim($_GET['id']);
    $qry = "select * from tallas where id=$id";
    //echo $qry."<br>";
...
<html lang="en">
    ...
    <form action="metodos.php?metodo=tallasmodificar" method="POST" onsubmit="return valida_campos()">
        <table class="styled-table-1">
            <thead>
                <tr>
                    <th colspan="2">MODIFICAR DATOS DE LA TALLA</th>
                </tr>
            </thead>
            <tbody>
                <tr>
                    <td>DESCRIPCION:</td>
                    <td><input type="text" id="descripcion" name="descripcion" value="<?php echo $descripcion?>" size="5" maxlength="5" oninput="convertirDescripcionAMayusculas()" style="text-transform: uppercase;">
        ...
        <table class="tablabotones">
            <tr>
                ...
                <td><input type="button" name="cancelar" id="cancelar" class = "boton" value="CANCELAR" title="CANCELAR"  onclick="javascript:{lanzar('tallasvista.php')}"></td>
            </tr>
        </table>
    </form>
</html>